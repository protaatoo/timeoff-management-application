...
router.get('/general/', async (req, res) => {
  ...
  res.render('general_settings', {
	...
	smtpConfig: config.email_transporter,
	application_sender_email: config.application_sender_email,
  });
});
...
router.post('/general', async (req, res) => {
  try {
    // Extract SMTP settings from the request body
    const { host, port, secure, user, pass, sender_email } = req.body;

    // Validate and prepare SMTP configuration
    const smtpConfig = {
      host,
    ...
    config.application_sender_email = sender_email;
    config.email_transporter = smtpConfig;

    // Write the updated configuration back to the file
    await fs.promises.writeFile(path.join(__dirname, '../config/app.json'), JSON.stringify(config, null, 2));

    // Flash success message and redirect
    req.session.flash_message('SMTP settings were successfully updated');
    res.redirect('/settings/general/');
  } catch (error) {
    // Log the error and flash an error message
    console.error(`An error occurred while updating SMTP settings for user ${req.user.id}: ${error} at ${error.stack}`);
    req.session.flash_error('Failed to update SMTP settings, please contact customer service');
    res.redirect('/settings/general/');
  }
});
