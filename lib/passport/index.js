...
function prepare_user_for_session(args) {
  ...
  user.maybe_activate()
    ...
    .then(function(){
	  var sessionDuration = rememberMe ? 7 * 24 * 60 * 60 * 1000 : 1 * 60 * 60 * 1000; // 7 days or 1 hour
      done(null, user, { maxAge: sessionDuration });
    });
}
...
function strategy_handler(email, password, done, rememberMe) {
      ...
      user.getCompany()
        .then(function(company){
          ...
          authenticate_user({
			...
			rememberMe: rememberMe // Pass rememberMe to authenticate_user
          });
        });
    })
