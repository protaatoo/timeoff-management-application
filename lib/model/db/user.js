...
function get_instance_methods(sequelize) {
  ...
  return {
    ...
    if (self.is_admin()) {
        // If the current user is an admin, fetch all users from the company
        const company = await self.getCompany({ scope: ['with_all_users'] });
        users = company.users;
    } else {
        // If not an admin, get all users from supervised departments plus the user themselves
        const supervisedDepartments = await self.promise_supervised_departments();
        
        // Use reduce to flatten the array of users
        users = supervisedDepartments.reduce((acc, department) => {
            return acc.concat(department.users);
        }, []);
    }
    ...
    // Remove duplicate users by ID and sort them by lastname
