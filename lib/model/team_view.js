...
function flattenArray(arr, depth = Infinity) {
  return arr.reduce((acc, val) => {
    if (Array.isArray(val) && (depth > 0 || depth === Infinity)) {
...
return Promise.all(promise_users_and_leaves)
  .then(users_and_leaves => {
    

    // Flatten the array of arrays into a single array
    users_and_leaves = Array.isArray(users_and_leaves) 
      ? flattenArray(users_and_leaves, Infinity)
      : [];

    users_and_leaves = users_and_leaves.sort((a, b) => sorter(
  ...
  .catch(error => {
    console.error("Error occurred while processing users and leaves:", error);
